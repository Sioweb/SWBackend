...
class DC_Table extends \Contao\DC_Table
{
  ...
  protected function panel() {
    ...
    foreach (trimsplit(';', $GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['panelLayout']) as $strPanel)
    {
      ...
      foreach ($arrSubPanels as $strSubPanel)
      {
        ...
        // Add the panel ifit is not empty
        if($panel != '')
          $panels = $panel . $panels;
      }

      // Add the group ifit is not empty
      if($panels != '')
        $arrPanels[] = $panels;
    }
  ...
  protected function treeView()
  {
    ...
    // Toggle the nodes
    if(\Input::get('pta') == 'all') {
      $node = ($GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode'] == 5) ? $this->strTable.'_'.$table.'_tree' : $this->strTable.'_tree';

      $node = 'showHideArticles';
      // Expand tree
      if(!is_array($session[$node]) || empty($session[$node]) || current($session[$node]) != 1) {
        $session[$node] = array();
        $objNodes = $this->Database->execute("SELECT DISTINCT pid FROM " . $this->strTable . " WHERE pid>0");

        while($objNodes->next())
          $session[$node][$objNodes->pid] = 1;
      } else $session[$node] = array();

      $this->Session->setData($session);
      $this->redirect(preg_replace('/(&(amp;)?|\?)pta=[^& ]*/i', '', \Environment::get('request')));
    }

    // Return ifa mandatory field (id, pid, sorting) is missing
...
<p class="tl_empty">strTable "'.$table.'" can not be shown as tree!</p>';
    ...
    // Return ifthere is no parent table
...
<p class="tl_empty">Table "'.$table.'" can not be shown as extended tree!</p>';
    ...
    $_buttons = '&nbsp;';
    // Show paste button only ifthere are no root records specified
  ...
  protected function generateTree($table, $id, $arrPrevNext, $blnHasSorting, $intMargin=0, $arrClipboard=null, $blnCircularReference=false, $protectedPage=false, $blnNoRecursion=false)
  {
    ...
    // echo ($this->strTable != $table).') '.$objRow->title.': '.$this->strTable.' - '.$table.'<br>';
    
    $tplTree->childsAndArticles = $childsAndArticles;
    $tplTree->hasArticles = implode(' ',$tplClass);
    $tplTree->childs = $treeChilds;

    if($session['showHideArticles'][$objRow->pid] == 1 && $table == $this->strTable)
    ...
    // Shorten the label ifit is too long
